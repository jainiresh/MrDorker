...
const AiGenerator = () => {
  ...
  const { email, accessToken } = useSelector(
  ...
  useEffect(() => {
    async function fetchBalanceCredits() {
      let credits = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/report/balance?email=${email}`
      );
  ...
  return (
    <div className="flex flex-col justify-center items-center bg-gradient-to-br from-gray-900 to-black">
      ...
      <div className="shadows" style={{ maxWidth: "90vw" }}>
        <div className="mb-[2rem]">
          <h1 className="text-sm font-bold">
            Generate Your Reports Effortlessly.
          </h1>
          <h4 className="text-sm">- With the power of AI</h4>
        </div>
        ...
        >
          <label
            htmlFor="input"
            className="block text-sm font-semibold font-medium"
          >
        ...
        <div className="mb-3">
          <h2 className="text-sm font-semibold">Choose Vulnerability type :</h2>
          <div
        ...
        <div className="mb-6">
          <h2 className="text-sm font-semibold">Choose Headings</h2>
          <div
            className="flex flex-wrap gap-2"
            style={{ justifyContent: "center" }}
          >
            {AI_REPORT_HEADINGS.map((heading) => (
              <button
                ...
                className={`item ${
                  selectedHeadings.includes(heading.value) ? "selected" : ""
                }
                  }`}
              >
                {heading.label}
        ...
        <div className="mb-2">
          <label
            ...
            className="block text-sm font-semibold font-medium "
          >
            If the above options doesn't satisfy you, explain the vulnerability
            , along with it's details to generate your report :
          </label>
          <textarea
            id="input"
            onChange={(event) => handleOtherDetailsChange(event.target.value)}
        ...
        {isLoading && (
          ...
          >
            <div style={{ top: "50vh", left: "50vw", position: "fixed" }}>
              <Loader />
            </div>
          </div>
        )}
        ...
        {activeVulnerability && (
          <Modal onClose={closeModal}>
            ...
            {VULNERABILITY_OPTIONS[activeVulnerability]?.map(
              (option, index) => (
